...
import com.example.dell.univstarproject.model.bean.DateUtils;
...
public class BrainPowerAdapter extends RecyclerView.Adapter<BrainPowerAdapter.Holder>{
    List<DemoFirstBean.DataBean.ListBean> mList;
    private Context mContext;

    public BrainPowerAdapter(List<DemoFirstBean.DataBean.ListBean> mList, Context mContext) {
        this.mList = mList;
    ...
    public void onBindViewHolder(final Holder holder, int position) {
        ...
        if (!TextUtils.isEmpty(tRealname)) {
            ((Holder)holder).linear.setVisibility(View.VISIBLE);
            Glide.with(mContext).load(mList.get(position).getTPhoto()).into(((Holder) holder).tptho_img);;
            ((Holder) holder).reaname.setText((String)mList.get(position).getTRealname());
            holder.tintrf.setVisibility(View.VISIBLE);
            ((Holder) holder).tintrf.setText((String)mList.get(position).getTIntro());

            Glide.with(mContext).load(mList.get(position).getPhoto()).into(((Holder) holder).phto_img);
            ((Holder) holder).nick_name.setText(mList.get(position).getNickname());
            SimpleDateFormat sdf = new SimpleDateFormat("MM/dd");
            Date date = new Date(mList.get(position).getCreateDate());
            holder.create_date.setText(sdf.format(date));
            ((Holder) holder).source.setText("来自"+mList.get(position).getSource());
            ((Holder) holder).content.setText(mList.get(position).getContent());
            Glide.with(mContext).load(mList.get(position).getCoverImg()).into(((Holder) holder).play_music);
            ((Holder) holder).work_type.setText(mList.get(position).getWorksType());
            ...
            ((Holder)holder).linear.setVisibility(View.GONE);
            Glide.with(mContext).load(mList.get(position).getPhoto()).into(((Holder) holder).phto_img);
            ((Holder) holder).nick_name.setText(mList.get(position).getNickname());
            SimpleDateFormat sdf = new SimpleDateFormat("MM/dd");
            Date date = new Date(mList.get(position).getCreateDate());
        ...
        Glide.with(mContext).load(mList.get(position).getPhoto()).asBitmap().centerCrop().into(new BitmapImageViewTarget(((Holder) holder).phto_img) {
        ...
        Glide.with(mContext).load(mList.get(position).getTPhoto()).asBitmap().centerCrop().into(new BitmapImageViewTarget(((Holder) holder).tptho_img) {
